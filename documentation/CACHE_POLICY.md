...
### Admin User Configuration (Frontend):

**Environment Variable:** `REACT_APP_ADMIN_UID_LIST` (in `.env`)

```
REACT_APP_ADMIN_UID_LIST=MAehKb0ZaFV0ehaV7MXwInPJ41J3,rRkab8ZStgZ30qPb53vuD1Kl1Ou2
```

This list defines which Firebase UIDs have admin access. Users not in this list cannot view or modify cache policy settings, even with admin_panel role.

### Backend Authorization:

**Environment Variable:** `ADMIN_ROLE_UPDATE_PASSWORD` (optional, for special admin operations)

This is stored in the backend `.env` for additional security checks on critical operations.
...
- ✅ **Admin Access Control:** REACT_APP_ADMIN_UID_LIST controls who can modify settings
...
| Admin UID list (.env) | `REACT_APP_ADMIN_UID_LIST` |
